...
df_all_exps = pd.concat([pd.read_csv(dir_path+'/dataframes/SVM/training and test sets/test set idx for exp'+str(exp) + '.csv').drop(columns=['Unnamed: 0']) for exp in exps])
...
df_all_info.iloc[:,0:6] = pd.concat([pd.concat([df_all_exps]*num_nets), pd.concat([df_all_exps]*num_nets)])

## Fill in information:

# 1. Fill in net:
df_all_info.loc[:,'net'] = np.tile(np.repeat(np.arange(1,num_nets+1), df_all_exps.shape[0]), len(networks))
# Fill in epoch:
df_all_info.loc[:,'network'] = np.repeat(networks, df_all_exps.shape[0]*num_nets)
...
for epoch in epochs:
    for net in nets:
        file = 'SVM prediction of He untrained net'+str(net)+' relu'+str(relu)+' epoch'+str(epoch)+' '+str(num_units)+ ' '+selectivity+' units that are randomly drawn from distribution exp'+str(exp)+ ' Jan2023.csv'
        net_idx = df_all_info[(df_all_info['epoch']==epoch)&(df_all_info['net']==net)].index.to_numpy()
        df_all_preds = pd.concat([pd.read_csv(dir_path+'/dataframes/SVM_predictions/'+file).drop(columns=['Unnamed: 0']) for exp in exps])
